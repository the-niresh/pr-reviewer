...
- Create: `src/pr_reviewer/runner/repository_fallback.py`
...
### Task 7A - ✅ Wire the lifecycle decision into the webhook

⚠️ Found during Task 7. `handle_pull_request_event` is built and tested, but
`control_plane/app.py:56` still calls `enqueue_review_job` directly for every `pull_request`
event, so **no lifecycle decision runs in production**. Draft PRs are enqueued, closed PRs are
enqueued, and `review_jobs.draft` and `cancelled` have schema with no writers.

Deferred because wiring it changes `test_webhook`, whose `{"action": "opened"}` fixture cannot
build a full `GitHubDelivery`.

**Files:**
- Modify: `src/pr_reviewer/control_plane/app.py`
- Modify: `tests/test_webhook.py`

**TDD steps:**
- ⬜ Write failing tests that a draft `opened` event is ignored and a `closed` event cancels,
  observed through the HTTP webhook route rather than by calling the function directly.
- ⬜ Grow the `test_webhook` fixtures into full `pull_request` payloads. Do not weaken
  `GitHubDelivery` to accept a partial payload; the fixture is what is wrong, not the contract.
- ⬜ Add writers for `review_jobs.draft` and `cancelled`. A column with schema and no writer is
  the same defect class as an unreachable route.
- ✅ Wired and committed as `d9e3433`, suite 390. Drafts no longer enqueue, closed PRs cancel,
  `GitHubDelivery` was not relaxed, and `LeaseState` gained `cancelled`.
- ✅ `recover()` abandons on `cancelled` too, committed as `9fd2f4c`, suite 391. One shared local
  `abandoned` status: both reasons mean this runner no longer owns the work, and the hosted
  `review_jobs` row already carries the real reason. Cancelling a job already running is
  deliberately not here; it is carried into Task 11.
- ⚠️ The check is still enumerated (`== "invalid_or_expired" or == "cancelled"`). Adding
  `cancelled` slipping past an enumerated check is exactly what caused this bug, and the next
  value added will slip past the same way. `!= "active"` fails safe instead: an unknown status
  abandons, which is recoverable hosted-side, where a leaked `claimed` row is not.
...
### Task 9 - ⚠️ Eval contracts, mined candidates, and checked holdout set

**Files:**
- ⚠️ Not created: hosted migration `<timestamp>_eval_foundation.sql`. Nothing reads or
  writes eval tables. The harness runs in process from `datasets/public/eval_cases.jsonl`,
  and the Task 9 boundary guard forbids `evals/` from importing `db`. An unused table would
  also need `ALLOWLIST` entries for columns no code touches. Build it when eval runs must
  persist for trend tracking, and give it an owner outside `evals/` at that point.
...
⚠️ The file list above was wrong and is corrected here. `models/` is **runner-side** and holds
the provider adapters, because they hold the user's model key. The hosted ledger writers
`record_model_call` and `record_prompt_version` live in `events/`. There is no `runner/models/`.
Committed as `54f91d2`, suite 424. Apply `202608311600_prompt_registry_constraints.sql` to any
database this branch uses.

**Files:**
- Create: `src/pr_reviewer/models/provider.py` (runner-side)
- Create: `src/pr_reviewer/models/openai_provider.py` (runner-side)
- Create: `src/pr_reviewer/models/anthropic_provider.py` (runner-side)
- Create: `src/pr_reviewer/prompts/registry.py`
- Create: hosted migration `202608311600_prompt_registry_constraints.sql`
- Moved: `record_model_call` and `record_prompt_version` into `events/` (hosted ledger writes)
- Test: `tests/test_model_provider.py`
- Test: `tests/test_prompt_registry.py`

Adapters hold the user's model key, so `models/` is runner-side and holds them. The hosted
ledger writes (`record_model_call`, `record_prompt_version`) live in `events/` beside
`record_event`. The Task 6 `runner/cli/` vs `cli/` split does not apply here: that pair exists
because both planes genuinely need a CLI, whereas a ledger of model calls and a model provider
are different nouns that happen to share a word.
...
⚠️ Two corrections to the list below. `OmissionReason` was **not** created here; it already
existed in `contracts/github.py` from Task 7 and was reused. And `reviewer.py` was already the
CLI router, so it became `reviewer/__init__.py` plus a `__main__.py`, keeping the
`reviewer = pr_reviewer.reviewer:main` entry point working.

The packing unit is the **whole file**, decided from measurement rather than preference: across
58 commits and 472 changed files in this repo the largest single file patch was ~13,280 tokens
and zero files exceeded a 32k budget, while the two configured models leave 111,616 and 191,808
tokens. Partial-file packing would buy coverage that is not needed and would cost the guarantee
that an included file was fully seen. A single file that ever does exceed the budget appears as
`TOKEN_BUDGET` in `omitted_files`, which is the signal to revisit this.

Committed as `da4ff8c`, suite 446.
...
✅ Committed as `ee0a661`. Enforcement is a **type**, not a scan: `UntrustedText.__str__` and
`__format__` raise, so f-string, `str()`, concat, `%` and `.format` all fail at the write site,
and `wrap_untrusted` rejects a plain `str` with `TypeError`. The fence strips markers to a
**fixed point**, because a single `.replace()` pass can rebuild a marker from the halves
surrounding the one it removes; that escape was demonstrated putting attacker text outside the
fence before the fix. Nested splits to three levels now all yield exactly one BEGIN and one END.
...
✅ Committed as `19adf25`, suite 498. Model output parses into `FindingDraft`, which simply has
no `id`, `review_job_id`, `verified`, `verification_method`, `public_safe` or `status` field, so
the model cannot set them rather than being stripped afterwards. `candidate_from_draft` is the
only construction path.

⚠️ My earlier note here said to heartbeat the lease while a review runs. That was wrong for this
task and has been built the simpler way: the baseline makes one model call, so the checkpoint is
a single synchronous heartbeat immediately before it. No thread, no poll loop, so rule 7 never
comes into play. Task 16 generalises it to between steps once there is more than one call.

⚠️ **The baseline number is blocked, not produced.** `run_diff_only_baseline` raises
`BaselineBlocked` because the public dataset is one dev case and zero holdout. No cases were
invented to fill it. Task 12A fixed the miner (`c35c7d1`): 40 FoodSpector commits now yield 37 usable
candidates instead of one 5.2M character blob. The remaining blocker is human audit time, not
tooling, because a holdout case requires a named auditor by validator.
...
- ✅ ⚠️ Carried from Task 7A. `LeaseState.status` can be `cancelled`, and today nothing acts
  on it for a running job. `daemon.py:148` is the only consumer and it handles recovery only.
  `_run_claimed_job` calls `review()` as one blocking call with no checkpoint, which is why the
  interrupt was deferred to here rather than built blind. This task creates the first work long
  enough to be worth cancelling, so it owns the checkpoint: give the reviewer a cancellation
  point between model calls, heartbeat the hosted lease while a review runs, and stop on
  `cancelled` without spending the next model call. Rule 7 applies to the heartbeat loop, so it
  needs a hard deadline and must never poll in a way that can hang. Test that a PR closed
  mid-review stops before the following model call and records the real reason, not a dead lease.
...
✅ Committed as `be0cdb7`, merged in `1500b79`. Chunking is `ast.parse` for Python with identity
`path::qualified_name`, and 40-line windows with 10-line overlap elsewhere; each chunk records
which strategy produced it. One active generation per repo is enforced by a partial unique
index, so two active generations are unrepresentable rather than merely avoided. The migration
lives in `local_store/postgres_migrations/`, never hosted `db/migrations/`, because the chunks
are the user's source. Exact scan measured at 2.625 ms over 300 chunks against a 5 s budget, so
...
✅ Committed as `abbc975`, merged in `65f6c26`, suite 532. Off by default
(`RETRIEVAL_ENABLED_DEFAULT = False`); the holdout comparison still raises `BaselineBlocked`
because the dataset has zero holdout cases. Chunks lose the budget before the diff does, with
two tests pinning it. The event-spine write is **injected** (`record_selection`) rather than
imported: `retrieval` is runner-side and `events/record_event.py` imports `db.client`, so a
direct import would have reached hosted Neon transitively with a green suite. See Task 13B.
...
✅ Committed as `7367580`, merged in `514ee10`, suite 566. Recency decided by measurement:
20 commits back 51% of `src/*.py` blobs already differ from HEAD, yet `SandboxSpec` still has no
`network_mode`, so a chunk ages hard and a structural claim ages soft. `exp(-0.05 * days)`, with
`usable_profile` refusing past 14 days where the weight hits 0.50.

⚠️ Caught in review: a **missing** `confidence` defaulted to `EXTRACTED`, promoting unknown
edges to parsed facts and inflating every `SensitivityScore`. Now defaults to `INFERRED`.
...
- ✅ A new profile run that **contradicts** an existing claim retires the old one rather than
  storing both. Two contradictory claims about the same codebase are worse than neither, because a
  reader cannot tell which is current. Reference: `context-memory` by samiksha0shukla models this as
  a four-way write decision (ADD, UPDATE, REPLACE, NOOP) where REPLACE deletes the contradicted
  memory; clone at `/srv/claude/reference/context-memory`, see `utils/tool_call_system_prompt.py`.
- ✅ Consider a soft recency weight on profile claims rather than a hard commit-SHA cutoff. A profile
  ages gradually, unlike a code chunk which is either current or wrong. The same reference scores
  `similarity * importance * recency` with `exp(-0.05 * days_ago)`, applied only to time-bound
  memories and never to durable facts. Decide with a measurement, not by assumption.
...
✅ Committed as `6a0772f`, merged in `65f6c26`, suite 532. Tested against real Docker 29.2.1.
`SandboxSpec` is unchanged, so isolation flags remain unrequestable. `command_id` is pattern
constrained to `^[a-z][a-z0-9_]*$`, which makes a shell string unconstructable rather than
rejected at run time, and the image is digest pinned. Missing Docker probes first and returns
`inconclusive` with `route_to_human=True` without ever calling `run()`.

⚠️ My prompt for this task wrongly said `verification` was already in `RUNNER_SIDE_PACKAGES`.
It was in `CONTROL_PLANE_FORBIDDEN_TARGETS`, a different rule in the opposite direction.
Cursor checked and added the missing entry.
...
✅ Committed as `eb4126c`, suite 588. `notifications/` is runner-side and takes an **injected**
send callable; hosted `connectors/` imports `db.client`, so owning the transports there would
have failed the transitive guard. Unset `confidentiality` defaults to `restricted`. Restricted
content on an ordinary channel raises `RestrictedContentRefused` instead of being downgraded.
The hosted row keeps `endpoint_hash` only; the webhook URL never leaves the runner.
...
✅ Committed as `2e16b4a`, merged in `54a8f21`, suite 610.

⚠️ Step state is **local SQLite, not hosted Neon**, overriding this plan's file list. Cursor's
reason is better than the plan: a hosted table would either grow findings-adjacent text and break
the empty `HOSTED_EXEMPTIONS`, or invent a second `pending`/`running` that can disagree with
`review_jobs` under a crash. `workflow_runs.outcome` is `null or in ('completed','cancelled',
'failed')`, so the duplicate running state is unrepresentable rather than merely avoided.

`wait_for_artifact` uses a monotonic hard deadline and raises `TIMEOUT` (rule 7). The lease check
runs between steps, generalising the Task 11 single-call checkpoint; a superseded head SHA
cancels as `superseded`.
...
✅ Committed as `c55c611`, suite 626. The head SHA is checked **after** comments are built and
immediately before `submit`, so the window between finishing a review and calling the API is
closed; a moved head raises `StalePullRequestHead` and posts nothing.

Idempotency key is `owner/repo#pr@sha:policy`, carried in the body as
`<!-- pr-reviewer:post:{key} -->`. A timeout where GitHub actually accepted the review is
recovered through `list_reviews` on that marker, so a retry finds the existing review instead of
posting a second one. The test asserts zero new submissions on both calls, which is what proves
recovery rather than re-post.

`post_review` imports nothing hosted: `submit`, `list_reviews`, `render_hunks`, `record_post`
and `record_event` are all injected, the same pattern as `record_selection`. Restricted findings
produce **no** comment rather than a redacted one, tested both alone and mixed with public ones.
Review IDs go in a flat event payload, not a new Neon table.

⚠️ Caught late by Cursor: the hosted wrap recorded 201 for review creation, but that API returns
**200**. The token mint genuinely is 201. Both are now correct.
...
- ⚠️ Run the same holdout set with the one-agent and specialist paths.
...
### Task 23 - ⚠️ CI, release containers, and supply-chain checks
...
- ⚠️ Add dependency lock checks, secret scanning, container scanning, and generated-file checks.
...
### Task 25 - ⚠️ Versioned public installer and setup wizard
...
**TDD steps:**
- ⚠️ Test install in a clean Linux container from a versioned GitHub release asset.
