...
describe("failReviewJob", () => {
  ...
  it("marks a running job failed and stores the error in the event log", async () => {
    ...
    await expect(failReviewJob("job-1", "GitHub request failed")).resolves.toBeUndefined();
    ...
    expect(query).toHaveBeenNthCalledWith(
      ...
      ["job-1", "review_job_failed", JSON.stringify({ error: "GitHub request failed" })],
    );
  ...
  it("rolls back when the job is not running", async () => {
    ...
    await expect(failReviewJob("job-1", "GitHub request failed")).rejects.toThrow(
      "Review job is not running: job-1",
    );
